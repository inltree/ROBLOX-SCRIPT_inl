--!native
-- https://github.com/78n/SimpleSpy 50/50 这可能会出问题，但测试版就是这样！
...
local realconfigs = {
    logcheckcaller = false, -- 记录客户端触发的远程调用
    autoblock = false, -- [测试版] 智能检测并排除日志中的垃圾远程调用
    funcEnabled = true, -- 启用函数信息（在某些游戏中可能导致卡顿）
    advancedinfo = false, -- 显示更多远程信息
    --logreturnvalues = false, -- [测试版] 记录 RemoteFunction 的返回值
    supersecretdevtoggle = false -- 超级秘密开发者开关
}
        ...
        while true do
            if parent and parent.Parent == game then
                if SafeGetService(parent.ClassName) then
                    if lower(parent.ClassName) == "workspace" 键，然后
...
----- 附加功能 ----- (轻松为远程间谍添加或移除额外功能！)
--[[
    一些有用的信息：
        - 在此处添加您的函数，并通过 'newButton' 函数为其创建按钮
        - 提供的第一个参数是玩家点击运行函数的文本按钮 (TextButton)
        - 生成的脚本在 namecall 最初触发时生成并保存在 remoteFrame 对象中
        - 黑名单中的远程调用将在 namecall 中直接被忽略（减少延迟）
        - 'remoteFrame' 对象的属性：
            {
                Name: (字符串) 远程调用的名称
                GenScript: (字符串) 代码框中出现的生成脚本（在 namecall 触发时生成）
                Source: (实例 (LocalScript)) 触发/调用远程调用的脚本
                Remote: (实例 (RemoteEvent) | 实例 (RemoteFunction)) 被触发/调用的远程调用
                Log: (实例 (TextButton)) 用于该远程调用的按钮（与 'selected.Log' 相同）
            }
        - 全局变量列表：（如需更多信息或有建议添加，请联系 @exx#9394）
            - closed: (布尔值) GUI 当前是否最小化
            - logs: (表[remoteFrame]) 充满 remoteFrame 对象（属性如上所列）
            - selected: (remoteFrame) 当前选中的 remoteFrame（属性如上所列）
            - blacklist: (字符串[] | 实例[] (RemoteEvent) | 实例[] (RemoteFunction)) 黑名单名称和远程调用的数组
            - codebox: (实例 (TextBox)) 保存所有代码的文本框 - 经常清空
]]
-- 复制代码框的内容
newButton(
    "复制代码",
...
newButton(
    "复制远程路径",
    function() return "点击复制远程调用的路径" end,
    function()
...
function()
    ...
    if func then
        ...
        if typeoffunc ~= 'string' then
            ...
            info = {
                info = getinfo(func),
                constants = lclosure and deepclone(getconstants(func)) or "N/A -- 期望 Lua 闭包，得到 C 闭包",
            ...
            if configs.advancedinfo then
                ...
                info["advancedinfo"] = {
                    ...
                    Protos = lclosure and getprotos(func) or "N/A -- 期望 Lua 闭包，得到 C 闭包"
                }
        ...
        TextLabel.Text = "完成！函数信息由 SimpleSpy V3 序列化器生成。"
    else
...
newButton(
    "排除 (实例)",
    function() return "点击排除此远程调用。\n排除远程调用会使 SimpleSpy 忽略它，但它将继续可用。" end,
    function()
        if selected then
...
--- 将所有与选定远程调用同名的远程调用从远程间谍中排除
newButton(
    "排除 (名称)",
    function() return "点击排除所有具有此名称的远程调用。\n排除远程调用会使 SimpleSpy 忽略它，但它将继续可用。" end,
    function()
        if selected then
...
newButton(
    "阻止 (实例)",
    function() return "点击停止此远程调用发往服务器。\n阻止远程调用不会将其从 SimpleSpy 日志中删除，但它将不再发往服务器。" end,
    function()
        if selected then
...
--- 阻止所有与选定远程调用同名的远程调用发往服务器（仍会记录）
newButton("阻止 (名称)",function()
    return "点击停止具有此名称的远程调用发往服务器。\n阻止远程调用不会将其从 SimpleSpy 日志中删除，但它将不再发往服务器。" end,
    function()
        if selected then
...
newButton("反编译",
        ...
        if decompile then
            if selected and selected.Source then
                ...
                if not DecompiledScripts[Source] then
                    ...
                    xpcall(function()
                        local decompiledsource = decompile(Source):gsub("-- 使用 Synapse X Luau 反编译器反编译。","")
    ...
    --[[newButton( -- 此按钮功能已注释掉
        "返回值",
        function() return "获取远程调用的返回数据" end,
        function()
            if selected then
                local Remote = selected.Remote
                if Remote and Remote:IsA("RemoteFunction") then
                    ...
                    codebox:setRaw("期望 RemoteFunction，得到 "..(Remote and Remote.ClassName))
                end
            end
        end
    )]]
...
newButton(
    "禁用信息",
    function() return string.format("[%s] 切换函数信息（因为它在某些游戏中可能导致卡顿）", configs.funcEnabled and "已启用" or "已禁用") end,
    function()
        configs.funcEnabled = not configs.funcEnabled
        TextLabel.Text = string.format("[%s] 切换函数信息（因为它在某些游戏中可能导致卡顿）", configs.funcEnabled and "已启用" or "已禁用")
    end
)

newButton(
    "自动阻止",
    function() return string.format("[%s] [测试版] 智能检测并排除日志中的垃圾远程调用", configs.autoblock and "已启用" or "已禁用") end,
    function()
        configs.autoblock = not configs.autoblock
        TextLabel.Text = string.format("[%s] [测试版] 智能检测并排除日志中的垃圾远程调用", configs.autoblock and "已启用" or "已禁用")
        history = {}
        excluding = {}
    end
)

newButton("记录调用者",function()
    return ("[%s] 记录由客户端触发的远程调用"):format(configs.logcheckcaller and "已启用" or "已禁用")
end,
function()
    configs.logcheckcaller = not configs.logcheckcaller
    TextLabel.Text = ("[%s] 记录由客户端触发的远程调用"):format(configs.logcheckcaller and "已启用" or "已禁用")
end)

--[[newButton("记录返回值",function() -- 此按钮功能已注释掉
    return ("[测试版] [%s] 记录 RemoteFunction 的返回值"):format(configs.logcheckcaller and "已启用" or "已禁用")
end,
function()
    configs.logreturnvalues = not configs.logreturnvalues
    TextLabel.Text = ("[测试版] [%s] 记录 RemoteFunction 的返回值"):format(configs.logreturnvalues and "已启用" or "已禁用")
end)]]

newButton("高级信息",function()
    return ("[%s] 显示更多远程信息"):format(configs.advancedinfo and "已启用" or "已禁用")
end,
function()
...
if table.find({
    ...
    Background.Draggable = true -- 背景可拖动
    local QuickCapture = Instance.new("TextButton") -- 快速捕获
    local UICorner = Instance.new("UICorner") -- UI圆角
    ...
    QuickCapture.Draggable = true -- 可拖动
